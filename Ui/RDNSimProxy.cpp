...
namespace
{
	...
	struct EqualControllerType :
		public std::binary_function< const Entity*, int, bool >
	{
...
{
	const EntityFactory* ef = m_pimpl->m_world->GetEntityFactory();
	const ControllerBlueprint* cbp = const_cast< EntityFactory* >( ef )->
		GetControllerBP( RDNEBP::Rock.folder, RDNEBP::Rock.file );
...
int RDNSimProxy::PaperEBP() const
{
	const EntityFactory* ef = m_pimpl->m_world->GetEntityFactory();
	const ControllerBlueprint* cbp = const_cast< EntityFactory* >( ef )->
		GetControllerBP( RDNEBP::Paper.folder, RDNEBP::Paper.file );
...
int RDNSimProxy::ScissorEBP() const
{
	const EntityFactory* ef = m_pimpl->m_world->GetEntityFactory();
	const ControllerBlueprint* cbp = const_cast< EntityFactory* >( ef )->
		GetControllerBP( RDNEBP::Scissor.folder, RDNEBP::Scissor.file );
...
int RDNSimProxy::BuildingEBPFromType( int type )
{
	...
	switch( type )
	{
		case HQ_EC					: ebp = &RDNEBP::HQ; break;
	...
	const EntityFactory* ef = m_pimpl->m_world->GetEntityFactory();
	const ControllerBlueprint* cbp = const_cast< EntityFactory* >( ef )->
		GetControllerBP( ebp->folder, ebp->file );
...
#define EVENTENTITY(type) static_cast<const type&>( event ).m_pSpawner;
		case GE_BuildUnitStart		: e = EVENTENTITY( GameEvent_BuildUnitStart     ); break;
		case GE_BuildUnitComplete	: e = EVENTENTITY( GameEvent_BuildUnitComplete  ); break;
		case GE_BuildUnitCancel		: e = EVENTENTITY( GameEvent_BuildUnitCancel    ); break;
#undef EVENTENTITY
	}
	...
	if( event.GetType() == GE_RallyPointSet )
	{

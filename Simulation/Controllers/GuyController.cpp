...
{
	if( id == MovingExtInfo				::ExtensionID ) return static_cast<const MovingExtInfo*				>( this );
	if( id == AttackExtInfo				::ExtensionID ) return static_cast<const AttackExtInfo*				>( this );
	if( id == HealthExtInfo				::ExtensionID ) return static_cast<const HealthExtInfo*				>( this );
	if( id == CostExtInfo				::ExtensionID ) return static_cast<const CostExtInfo*				>( this );
	if( id == SightExtInfo				::ExtensionID ) return static_cast<const SightExtInfo*				>( this );
	if( id == UIExtInfo					::ExtensionID ) return static_cast<const UIExtInfo*					>( this );
...
{
	if ( id == HealthExt			::ExtensionID )	return static_cast< HealthExt*				>( this );
	if ( id == ModifierExt			::ExtensionID )	return static_cast< ModifierExt*			>( this );
	if ( id == MovingExt			::ExtensionID )	return static_cast< MovingExt*				>( this );
	if ( id == SightExt				::ExtensionID )	return static_cast< SightExt*				>( this );
	if ( id == AttackExt			::ExtensionID )	return static_cast< AttackExt*				>( this );
...
{
	if ( StateID == State			::SID_Current )	return m_CurrentState;

	if ( StateID == StateIdle			::StateID )	return &m_stateidle;
	if ( StateID == StateMove			::StateID )	return &m_statemove;
	if ( StateID == StateDead			::StateID )	return &m_statedead;
	if ( StateID == StateAttack			::StateID )	return &m_stateattack;
	if ( StateID == StateAttackMove		::StateID )	return &m_stateattackmove;
	if ( StateID == StateGroupMove		::StateID )	return &m_stategroupmove;
	if ( StateID == StatePause			::StateID )	return &m_statepause;
	return NULL;
}

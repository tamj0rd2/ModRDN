	...
	HQController::StaticInfo::QInfo( unsigned char id ) const
{
	...
	if( id == SiteExtInfo			::ExtensionID ) return static_cast<const SiteExtInfo*		>( this );
...
//	Author	: 
//
HQController::HQController( Entity *pEntity, const ECStaticInfo* si ) :
	ModController	( pEntity, SiteExtInfo::CreateDynamics( pEntity, QIExtInfo<SiteExtInfo>(si) ), si ), 
...
bool HQController::CommandDoProcessNow( const EntityCommand* ec )
{
	...
	if( ec->GetCommandType() == ec->CT_Entity && ec->GetCommand() == CMD_CancelBuildUnit )
	{
...
Extension* HQController::QIAll( unsigned char id )
{
	...
	if ( id == UnitSpawnerExt::ExtensionID )		return static_cast< UnitSpawnerExt* >( this );
...
{
	if ( stateid == State			::SID_Current )	return m_pCurrentState;
